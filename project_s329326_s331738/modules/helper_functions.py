...
def rounding_rule():
    """
    To do: rounding to 0 or 0.5 all number, it is forbidden to
    give for example 0.6/0.8 of star
    :return:
    """
...
def rounding():
    """
    To do: Rounding ratings with respect rounding_rule
    :return:
    """
...
def imputate_data_with_mean(df):
    ...
    return df.where(pd.notna(df), df.mean(), axis="columns")
